...
%     RawData      (D x NSamples double) series of data with power line hum
%                  (If  NSamples x D,  this is corrected automatically)
...
%
% OUTPUT
%     Filtered  (D x NSamples double) clean data
%     Filter    (D x NSamples double) shape of the subtraction filter
...
% if the data are of NSamples x D, invert the data
if Dims(1)>Dims(2)
    InvertData = true;
    RawData = RawData';
else
...
% if the data are of NSample x D, invert the data to the original dimension
if InvertData == true
    Filtered = Filtered';
end
end
